...
target_encoder = joblib.load('target_encoders5.pkl')  # LabelEncoder trained on ['Low', 'Medium', 'High']
...
with col2:
    ...
    with st.expander("📄 View Data"):
        ...
        st.table(tab)
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "wlpa_schedule.csv", "text/csv")
...
with col3:
    ...
    with st.expander("📄 View Data"):
        ...
        st.table(tab)
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "current_status.csv", "text/csv")
...
with col5:
    ...
    with st.expander("📄 View Data"):
        ...
        st.table(tab)
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "habitat_type.csv", "text/csv")
...
with col6:
    ...
    with st.expander("📄 View Data"):
        ...
        st.table(tab)
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "diet_type.csv", "text/csv")
...
with col7:
    ...
    with st.expander("📄 View Data"):
        tab = df[['long_term_trend', 'current_annual_change', 'long_term_status']].copy()
        st.table(tab.head(20))  # show first 20 rows for readability
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "long_term_vs_current.csv", "text/csv")
...
with col8:
    ...
    with st.expander("📄 View Data"):
        tab = df['endemicity_type'].value_counts().reset_index()
        st.table(tab)
        csv = tab.to_csv(index=False).encode()
        st.download_button("Download CSV", csv, "endemicity_type.csv", "text/csv")
